...
my $maxErr = int($error * $maxDist);
$sites[0] = IntegerInterval->new(0,0);
$sites[-1] = IntegerInterval->new($maxDist-$maxErr,$maxDist+$maxErr);
$d[0][$#sites] = $distances[-1];
$d[$#sites][0] = $distances[-1];
...
sub checkGlobalConstraints {
	...
	foreach my $i (0..$j-1) {
	    $revSites[$j] = 
		$revSites[$j]->
		    intersect($revSites[$i]->plus($d[$i][$j]));
	    $revSites[$j] or return undef;
	}
    }
    return \@revSites;
}
